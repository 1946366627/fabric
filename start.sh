...
#安装链码，只需要在背书节点安装
docker exec cli bash -c "$peer0org1 peer chaincode install -n evidence -v 1.0 -l golang -p github.com/chaincode"
docker exec cli bash -c "$peer0org2 peer chaincode install -n evidence -v 1.0 -l golang -p github.com/chaincode"

#实例化链码，链码必须实例化后才可以使用 如果是开发模式建议使用OR策略实例化链码
docker exec cli bash -c "$peer0org1 peer chaincode instantiate -o orderer.example.com:7050  -C mychannel -n evidence -l golang -v 1.0 -c '{\"Args\":[\"test\",\"链码实例化成功\"]}' -P 'AND ('\''Org1MSP.peer'\'','\''Org2MSP.peer'\'')'"
sleep 5
#在peer0org1上query测试
docker exec cli bash -c "peer chaincode query -C mychannel -n evidence -c '{\"Args\":[\"queryVideo\",\"test\"]}'"

#在peer0org2上invoke测试
docker exec cli bash -c "peer chaincode invoke -o orderer.example.com:7050 -C mychannel -n evidence --peerAddresses peer0.org1.example.com:7051 --peerAddresses peer0.org2.example.com:9051 -c '{\"Args\":[\"videoUplink\",\"alicepk\",\"bobpk\",\"CID123\",\"video.mp4\"]}'"
sleep 5
#在peer1org1上安装并query测试
docker exec cli bash -c "peer chaincode query -C mychannel -n evidence -c '{\"Args\":[\"queryUplink\",\"alicepk\"]}'"
...
#设置IPFS webui可以访问，访问地址为：ip:6001/webui
#blockchain-ipfs-1 是kubo的容器名称，可以用docker ps命令查看
docker exec blockchain-ipfs-1 ipfs config --json API.HTTPHeaders.Access-Control-Allow-Origin '["*"]'
docker exec blockchain-ipfs-1  ipfs config --json API.HTTPHeaders.Access-Control-Allow-Methods '["PUT", "POST"]'
#重启后生效
docker restart blockchain-ipfs-1
